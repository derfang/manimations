...
def preditor_prey_vector_field(point):
    ...
    return rotate_vector(result, 1 * DEGREES)
...
class TeacherStudentsScene(Scene):
    # Define the TeacherStudentsScene class or import it if available in another module
    pass
...
from pi_creature_scene import PiCreatureScene
...
class DivergenceAtSlowFastPoint(Scene):
    ...
    def construct(self):
        ...
        slow_words = Tex("Slow flow in")
        fast_words = Tex("Fast flow out")
...
class IllustrationUseVennDiagram(Scene):
    def construct(self):
        ...
        illustrated_by = Tex("Illustrated by")
...
class MaxwellsEquations(Scene):
    ...
    def add_equations(self):
        ...
        field_definitions = VGroup(*[
            MathTex(text, tex_to_color_map=tex_to_color_map)
            for text in [
...
class IllustrateGaussMagnetic(IllustrateGaussLaw):
    ...
    def add_wires(self):
        ...
        top.add(MathTex("\\times").scale(0.5))
...
class PhaseSpaceOfPopulationModel(ShowTwoPopulations, PiCreatureScene, MovingCameraScene):
    ...
    def write_differential_equations(self):
        ...
        title = Tex("Differential equations")
...
class QuickWordsOnNotation(Scene):
    def construct(self):
        words = Tex("Quick words on notation:")
        words.scale(1.5)
        self.play(FadeInFromDown(words))
        self.wait()
...
class NablaNotation(PiCreatureScene, MovingCameraScene):
    ...
    def zoom_out(self):
        ...
        words = Tex("Something deeper at play...")
        words.scale(1.3)
...
class DivergenceTinyNudgesView(MovingCameraScene):
    ...
    def zoom_in(self):
        ...
        input_words = Tex("$(x_0, y_0)$").scale(sf)
        input_words.next_to(input_dot, DL, SMALL_BUFF * sf)
        output_words = Tex("Output").scale(sf)
    ...
    def take_tiny_step(self):
        ...
        new_output_words = Tex("New output").scale(sf)
        ...
        small_step_words = Tex("(think tiny step)").scale(sf)
        ...
        diff_words = Tex("Difference").scale(sf)
...
class IncmpressibleAndIrrotational(Scene):
    def construct(self):
        div_0 = Tex("div$\\textbf{F} = 0$")
        curl_0 = Tex("curl$\\textbf{F}$ = 0")
        incompressible = Tex("Incompressible")
        irrotational = Tex("Irrotational")
        ...
        question = Tex("Does this actually happen?")
...
class NoChargesOverlay(Scene):
    def construct(self):
        ...
        words = Tex("No charges outside wire")
        words.scale(1.5)
        words.to_edge(UP)
...
class BroughtToYouBy(PiCreatureScene):
    ...
    def brought_to_you_by(self):
        so_words = Tex("So", "...", arg_separator="")
        so_words.scale(2)

        btyb = Tex("Brought to you", "by")
        btyb.scale(1.5)
    ...
    def just_you_and_me(self):
        ...
        for pi in self.pi_creatures:
            pi.change("pondering")
        math = MathTex("\\sum_{n=1}^\\infty \\frac{1}{n^s}")
        math.scale(2)
...
class ThoughtsOnAds(Scene):
    def construct(self):
        ...
        misaligned = Tex("Misaligned")
        misaligned.next_to(arrows[0], DOWN)
        aligned = Tex("Well-aligned")
...
class HoldUpPreviousPromo(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.play(
             FadeIn(screen_rect, direction=DOWN),
           morty.change, "raise_right_hand",
        )
        self.wait(5)
...
class PeopleValueGraph(GraphScene):
    ...
    def comment_on_incentives(self):
        ...
        reach_words = Tex("Maximize reach?")
        ...
        area_words = Tex("Maximize this area")
